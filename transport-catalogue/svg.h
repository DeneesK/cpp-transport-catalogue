...
struct Rgb {
    Rgb(uint16_t red_, uint16_t green_, uint16_t blue_): red(red_), green(green_), blue(blue_)
    {
    }
    Rgb() {
        red = 0;
...
struct Rgba {
    Rgba(uint16_t red_, uint16_t green_, uint16_t blue_, double opacity_): red(red_), green(green_), blue(blue_), opacity(opacity_)
    {
    }
    Rgba() {
        red = 0;
...
protected:
    ...
    void RenderAttrs(std::ostream& out) const {
        ...
        if(std::visit(ColorChecker{}, fill_color_)) {
            out << " fill=\""sv << fill_color_ << "\""sv;
        }
        if(std::visit(ColorChecker{}, stroke_color_)) {
...
private:
    ...
    std::string data_ = "";
    std::unordered_map<char, std::string> need_to_escape_ = {{'\"', "&quot;"}, {'\'', "&apos;"}, {'<',"&lt;"}, {'>',"&gt;"}, {'&',"&amp;"}};
    
    void RenderObject(const RenderContext& context) const override;
    void DoEscapingСharacters(std::string& data); 
};
