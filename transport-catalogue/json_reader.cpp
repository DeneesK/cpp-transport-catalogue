...
namespace json_reader {
    ...
    request_handler::Requests LoadJSON(std::istream& input, catalogue::TransportCatalogue& db) {
        ...
        json::Node stat_requests = dict.at("stat_requests");

        ProceedDBRequests(base_requests, db);

        request_handler::Requests requests;
        
        for(auto req: stat_requests.AsArray()) {
            auto request = req.AsMap();
            auto id = request.at("id").AsString();
    ...
    void ProceedDBRequests(json::Node& req, catalogue::TransportCatalogue& db) {
        ...
        for(json::Node elem: req.AsArray()) {
            if(elem.AsMap().at("type") == "Bus") {
